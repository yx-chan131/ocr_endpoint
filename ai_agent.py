...
def unsupported_document_type():
    """Use this tool if document type is not supported (referral letter, medical certificate, receipt)"""
    return False
...
def extract_referral_letter(
    ...
    - signature_presence: No need to check for this, False by default
...
def create_ai_agent(openai_api_key, verbose=False):
    tools = [extract_referral_letter, extract_medical_certificate, extract_receipt, unsupported_document_type]
    # Initialize the LLM
    llm = ChatOpenAI(
        temperature=0.2,
        model_name="gpt-4o",
...
if __name__ == "__main__":
    agent = create_ai_agent(os.getenv("OPENAI_API_KEY"), verbose=True)
    ...
    """
    response = agent.run(medical_certificate_input)
    print(response)
